...
description: >
  专门抓取 X/Twitter 链接内容：单条推文、长推、X Articles（长文）、回复楼、用户时间线。
  当用户发送 x.com / twitter.com 链接，或要求“读取/提取/总结/抓取某条推文、某篇 X 文章、某个账号时间线”时，优先使用这个 skill。
  也可用于补充抓取回复、监控 mentions、按关键词发现推文。基础单条抓取零依赖；回复/时间线/搜索依赖 Camofox。
---
