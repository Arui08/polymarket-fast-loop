...
# Binance Square 发文规则
- **API 配置已永久存储**: API 密钥 `ac5737618b614158a03f095995a83bb6` 已配置于 `config/binance-square.env`，后续所有发文任务直接使用，不再询问用户。
...
- 发文前必须检查币安广场字数限制；若超限，自动压缩后重发，优先保留关键价格位、方向判断、热点、meme/X 热议、聪明钱、操作思路。
- 关键交易数据必须尽量补齐，尤其是 BTC / ETH / BNB 当前价格、关键支撑位/压力位、在哪个点位看多、在哪个点位看空、突破后路径、跌破后路径、未来 1-2 个交易阶段预判；不能再发缺少关键位和方向判断的空泛版本。
...
- 文章必须尽量按以下 6 段组织，缺一不可：
  ...
  2. 关键价格区间判断：明确写出关键位/区间，并判断“上方偏多 / 下方偏空 / 突破或跌破后大概率怎么走”；同时尽量补齐当前价格、支撑位/压力位、看多/看空触发点、未来 1-2 个交易阶段预判。
...
## 自动整合 2026-03-11 2026-03-11 09:19（北京时间）
