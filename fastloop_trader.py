...
def _parse_resolves_at(resolves_at_str):
 ...
 return Nonedef _parse_fast_market_end_time(question):
...
COINGECKO_ASSETS = {"BTC": "bitcoin", "ETH": "ethereum", "SOL": "solana"}def get_momentum(asset="BTC", source="binance", lookback=5):
...
def run_fast_market_strategy(dry_run=True, positions_only=False, show_config=False,
 ...
 returnend_time = best.get("end_time")
 ...
 log(f" ✅ {'[PAPER] ' if result.get('simulated') else ''}Bought {shares:.1f} {side.upper()} shares @ ${price:.3f}", force=True)# Update daily spend (skip for paper trades)
...
if name == "__main__":
 ...
 args = parser.parse_args()if args.set:
 updates = {}
